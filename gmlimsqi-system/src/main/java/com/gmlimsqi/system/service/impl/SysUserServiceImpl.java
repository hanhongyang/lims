...
import com.gmlimsqi.common.core.domain.entity.SysRole;
...
import com.gmlimsqi.common.utils.bean.BeanValidators;
...
public class SysUserServiceImpl implements ISysUserService
{
    ...
    public SysUser selectUserByPhonenumber(String username) {
        return userMapper.selectUserByPhonenumber(username);
    }

    @Override
    public SysUserSign selectUserSign() {
        ...
        if (StringUtils.isBlank(sysUser.getSign()))
        {
            throw new ServiceException("用户未上传签名");
        }

        SysUploadFile sysUploadFile = uploadFileMapper.selectFileById(sysUser.getSign());

        if (sysUploadFile == null)
        {
            throw new ServiceException("用户签名文件不存在");
        }

        SysUserSign sysUserSign = new SysUserSign();
        sysUserSign.setUserId(userId);
        sysUserSign.setSign(sysUser.getSign());
        sysUserSign.setSignUrl(sysUploadFile.getUrl());
        return sysUserSign;
    }

    @Override
    public void updateUserSign(SysUserSign sysUserSign) {

        SysUser sysUser = userMapper.selectUserById(sysUserSign.getUserId());
        ...
        sysUser.setSign(sysUserSign.getSign());
